...
def test_domain_selection_integration():
    # 1. 加载真实RCT文章数据
    ...
    try:
        ...
        for q_id in ["q5_1", "q5_2", "q5_3"]:  # Selection domain的特定信号问题
...
def test_domain_selection_error_cases():
    """测试错误处理"""
    agent = DomainSelectionAgent(
        model_name="gemma3",
        model_provider=ModelProvider.OLLAMA,
    ...
    try:
        ...
        minimal_content = [
            {
                "type": "text",
                "text": "The trial adhered to a pre-specified analysis plan.",
                "page_idx": 0,
            }
        ]
